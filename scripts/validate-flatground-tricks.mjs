...
const sourcePath = new URL("../notes/projects/flatground-tricks-source.csv", import.meta.url);
...
const expectedHeader=["trick_number","trick_name","stance","part","caption_seconds","family","subfamily","modifiers","sibling_key","youtube_url"];
const header=sourceRows.shift();
if(JSON.stringify(header)!==JSON.stringify(expectedHeader)) throw new Error("Unexpected source CSV header");
const sourceData=sourceRows.map(r=>({
  num:Number(r[0]),name:r[1],stance:r[2],part:Number(r[3]),seconds:Number(r[4]),
  family:r[5],subfamily:r[6],modifiers:r[7]?r[7].split("|").filter(Boolean):[],siblingKey:r[8],url:r[9]
}));
...
if(JSON.stringify(data)!==JSON.stringify(sourceData)) throw new Error("Generated JS data does not match source CSV");
...
const inlineScripts=[...htmlSource.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)].map(m=>m[1]).filter(Boolean);
if(!inlineScripts.length) throw new Error("No inline application script found");
for(const [i,source] of inlineScripts.entries()){
  try { new vm.Script(source,{filename:`flatground-tricks-inline-${i+1}.js`}); }
...
if(![800,900,1000].includes(data.length)) fail(`Expected 800, 900, or 1000 tricks; found ${data.length}`);
const expectedParts=data.length/100;
const numbers=new Set(),partLast=new Map(),partCounts=new Map();
for(let i=0;i<data.length;i++){
  const d=data[i], expected=i+1, expectedPart=Math.floor(i/100)+1;
  if(d.num!==expected) fail(`Expected trick #${expected}, found #${d.num}`);
  if(numbers.has(d.num)) fail(`Duplicate trick number #${d.num}`); numbers.add(d.num);
  if(d.part!==expectedPart) fail(`#${d.num} must be Part ${expectedPart}; found Part ${d.part}`);
  partCounts.set(d.part,(partCounts.get(d.part)||0)+1);
  if(!d.name) fail(`Missing name at #${d.num}`);
  ...
  for(const mod of d.modifiers) if(!allowedModifiers.has(mod)) fail(`Unknown modifier at #${d.num}: ${mod}`);
  if(!d.siblingKey) fail(`Missing sibling key at #${d.num}`);
}
for(let p=1;p<=expectedParts;p++) if(partCounts.get(p)!==100) fail(`Part ${p} must contain exactly 100 tricks`);
...
for(const start of Object.keys(aliases)){
  const seen=new Set(); let key=start;
  while(aliases[key]){ if(seen.has(key)) fail(`Sibling alias cycle starting at: ${start}`); seen.add(key); key=aliases[key]; }
}
const canonicalSiblingKey=key=>{const seen=new Set();while(aliases[key]&&!seen.has(key)){seen.add(key);key=aliases[key]}return key};
...
const formatTimestamp=seconds=>{const m=Math.floor(seconds/60);return `${m}:${(seconds-m*60).toFixed(1).padStart(4,"0")}`};
...
if(csvSource!==expectedCsv) fail("Public CSV does not match canonical source");

const desc=`A sortable digital compendium of ${data.length} flatground tricks performed by Jamie Griffin.`;
if(!htmlSource.includes(desc)) fail("Static description count is out of date");
if(!htmlSource.includes(`aria-label="${data.length}-trick stance overview"`)) fail("Overview count label is out of date");
if(!htmlSource.includes(`>${data.length} / ${data.length}</span>`)) fail("Initial result count is out of date");
...
console.log(`Flatground Tricks validation passed: ${data.length} tricks, ${expectedParts} parts, ${siblingStances.size} sibling groups.`);
